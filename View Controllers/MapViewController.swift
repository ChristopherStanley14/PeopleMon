...
class MapViewController: UIViewController, SegueHandlerType {
    ...
    @IBAction func nearby(_ sender: Any) {
        let person = User(radiusInMeters: Int(Int(Constants.nearbyRadius))
            ...
            if let objects = objects {
                ...
                for user in objects {
                    if let latitude = user.latitude, let longitude = user.longitude {
                        user.distance = self.locationManager.location?.distance(from: CLLocation(latitude: latitude, longitude: longitude))
                    }
                }
...
extension MapViewController: MKMapViewDelegate {
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        if annotation is MKUserLocation {
            ...
            if let image = Utils.imageFromString(imageString: UserStore.shared.user?.avatarBase64) {
    ...
    func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
        ...
        if let userOverlay = userOverlay, userOverlay.coordinate.latitude == overlay.coordinate.latitude && userOverlay.coordinate.longitude == overlay.coordinate.longitude {
            ...
            circleOverlay.strokeColor = UIColor.gray
            circleOverlay.fillColor = UIColor.gray.withAlphaComponent(0.5)
        } else {
            circleOverlay.lineWidth = 0
            circleOverlay.fillColor = UIColor.gray.withAlphaComponent(0.25)
        }
